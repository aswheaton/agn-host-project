...
lognormal1 = {}
lognormal1["tmax"] = (3.5, 10.5)
lognormal1["fwhm"] = (0.1, 5.0)
lognormal1["massformed"] = (0.0, 15.0)
lognormal1["metallicity"] = (0.0, 2.5)
...
for filename in datafiles:
    ...
    sigma_values = {"exponential" : np.percentile(fit.posterior.samples["age"], 84) - np.percentile(fit.posterior.samples["age"], 16)}
    ...
    sigma_values["delayed"] = np.percentile(fit.posterior.samples["age"], 84) - np.percentile(fit.posterior.samples["age"], 16)
    fit.fit(verbose=True)
    ...
    sigma_values["lognormal"] = np.percentile(fit.posterior.samples["tmax"], 84) - np.percentile(fit.posterior.samples["tmax"], 16)
    fit.fit(verbose=True)
